...
sudo chmod +x dial.txt dial.sh init_dial.txt
sleep 1
...
minicom -D /dev/ttyUSB2 -S /home/$(logname)/init_dial.txt
sleep 1
...
sudo su -c "7z x SIM7600_NDIS.7z   -r -o./SIM7600_NDIS -y"
sleep 1
# Reset make files before compiling
sudo su -c "cd /home/$(logname)/SIM7600_NDIS && make clean"
sleep 1
# Run make files to compile SIM Hat driver
sudo su -c "cd /home/$(logname)/SIM7600_NDIS && make"
sleep 1
# Run dial.sh to configure internet data call
sudo sh /home/pi/dial.sh
sleep 1
# Check whether the command line is already exist in /etc/rc.local
if ! sudo grep -q "sudo sh /home/$(logname)/dial.sh &" /etc/rc.local; then
    # Append the file into /etc/rc.local (1 line above "exit 0") to enable automatic run after reboot 
    sudo sed -i "$(($(wc -l < /etc/rc.local)-0))i sudo sh /home/$(logname)/dial.sh &" /etc/rc.local
fi
sleep 1
